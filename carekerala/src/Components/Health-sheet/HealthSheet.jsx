...
function HealthSheet({ hSheet }) {
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      const res = await axios.post(`${import.meta.env.VITE_BASE_URL}/h-sheet/update/user/` + userId,data,
        {
          headers: {
      ...
      {isEditing ? (
        <div className={styles.sheetInnerDiv1}>
          ...
          <div className={styles.flexCol}>
            ...
            {editedDetails.emergencyContacts.map((emergencyContact, index) => (
            ...
            {editedDetails.emergencyContacts.length > 1 && (
              <button onClick={handleRemoveContact} className={styles.btnSelfRed}>Remove Contact</button>
            )}
            <button onClick={handleAddContact} className={styles.btnSelfDark}>+ Add Contact</button>
          </div>
        </div>
      ) : (
        <div className={styles.sheetInnerDiv1}>
          ...
          <div className={styles.flexCol}>
            ...
            <span>
              Phone Number :{" "}
              {editedDetails.personalInformation.contactInformation.phoneNumber}
            </span>
            <h5>Emergency Contacts</h5>
          ...
          <div className={styles.flexCol}>
            <h5>Last Updated</h5>
            <span>Doctor : {hSheet.lastUpdated.doctor}</span>
            <span>
              hospital : {hSheet.lastUpdated.hospital}{" "}
        ...
        <div className={styles.sheetDetails}>
          <div className={styles.editingSheetDetails}>
            ...
            <div className={styles.editingDivCol}>
              <h5>Allergies</h5>
              {editedDetails.medicalHistory.allergies.map((allergy, index) => (
                <div key={index} className={styles.flexCol}>
                  <input
              ...
              <button onClick={() => handleAddMedicalHistory("allergies")} className={styles.btnSelfDark}>
               + Add Allergy
              </button>
            </div>
            ...
            <div className={styles.editingDivCol}>
              ...
              <button
                onClick={() => handleAddMedicalHistory("preExistingConditions")} className={styles.btnSelfDark}
              >
            ...
            {editedDetails.medicalHistory.surgeries.map(
              (surgeriesItem, index) => (
                <div key={index} className={styles.flexCol}>
                  ...
                  <button onClick={() => handleRemoveSurgeries(index)} className={styles.btnSelfRed}>
                    Remove
                  </button>
                </div>
              )
            )}
            <button onClick={handleAddSurgeries} className={styles.btnSelfDark}>+ Add Surgery</button>
            </div>
          ...
          <input
            className={styles.wFull}
            type="text"
            value={editedDetails.healthGoalsAndLifestyle.dietaryHabits || ""}
            onChange={(e) =>
              setEditedDetails((prevData) => ({
                ...prevData,
                healthGoalsAndLifestyle: {
          ...
          <input
            type="text"
            value={editedDetails.healthGoalsAndLifestyle.exerciseRoutine || ""}
            onChange={(e) =>
              setEditedDetails((prevData) => ({
                ...prevData,
                healthGoalsAndLifestyle: {
          ...
          {editedDetails.healthGoalsAndLifestyle.healthGoals.map(
            (goal, index) => (
              <div key={index} className={styles.flexCol}>
                ...
                />
                <button onClick={() => handleRemoveHealthGoal(index)} className={styles.btnSelfRed}>
                  Remove Health Goal
                </button>
              </div>
            )
          )}
          <button onClick={handleAddHealthGoal} className={styles.btnSelfDark}>+ Add Health Goal</button>
            </div>
           
          </div>
          <div className={styles.bottomBtns}>
          <button onClick={handleCancelEdit} className={styles.btnCancel}>Cancel</button>
          <button onClick={handleSaveChanges} className={styles.btnSave}>Save</button>
          </div>
        ...
        <div className={styles.sheetDetails}>
          ...
          <div className={styles.flexCol}>
            ...
            <span className={styles.flexRow}>
              ...
              {hSheet.medicalHistory.preExistingConditions?.map((d, index) => {
          ...
          <div>
            <h3>Vital Signs</h3>
            <div>
              {hSheet.vitalSigns.map((d, index) => {
                return (
                  <li key={index} className={styles.listCol}>
                    <span>Blood Pressure : {d.bloodPressure}</span>
                    <span>Body Temperature : {d.bodyTemperature}</span>
                    <span>Heart Rate : {d.heartRate}</span>
                    <span>Respiratory Rate : {d.respiratoryRate}</span>
                  </li>
                );
              })}
            </div>
          ...
          <div>
            ...
            {hSheet.appointmentsAndVisits?.length > 0 ? (
              <ul>
                {hSheet.appointmentsAndVisits.map((d, index) => {
                  return (
                    <li key={index}>
                      <span>Name : {d.name}</span>
                      <span>Dosage : {d.dosage}</span>
              ...
              <p>No Appointments And Visits available.</p>
            )}
          </div>
          ...
          <div>
            ...
            {hSheet.healthGoalsAndLifestyle.healthGoals?.length > 0 ? (
              <p>
                {hSheet.healthGoalsAndLifestyle.healthGoals.map((d, index) => {
                  return <li key={index}>{d}</li>;
                })}
              </p>
            ) : (
